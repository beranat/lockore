...
/**
 * Help page - use HelpIndexScreen for showing help
 */
final class HelpScreen extends Form implements CommandListener{
    protected final Displayable parent_;

    protected HelpScreen(Displayable parent, String title, String topic, boolean fromIndex) {
        super(title);
        parent_ = parent;

        if (!fromIndex)
            addCommand(new Command(I18N.get("Content"), Command.HELP, 1));
    ...
    public void commandAction(Command c, Displayable d) {
        Displayable next = parent_;
        if (c.getCommandType() == Command.HELP)
            next = new HelpIndexList(parent_);
